...
export class DashboardComponent implements OnInit {
 ...
 dpsPhysicalYE3 = [];
  ...
  getSummary() {
    ...
    this.pmisService.getTblMFO().subscribe((data: any) => {
      ...
      for (let i in groupBy) {
        ...
        for (let x in groupBy[i]) {
          var sumTarget = groupBy[i][x].jant + groupBy[i][x].febt + groupBy[i][x].mart  + groupBy[i][x].aprt + groupBy[i][x].mayt + groupBy[i][x].junt;
          var sumAccomp = groupBy[i][x].jana + groupBy[i][x].feba + groupBy[i][x].mara  + groupBy[i][x].apra + groupBy[i][x].maya + groupBy[i][x].juna;
          var sumTarget2 = sumTarget + groupBy[i][x].jult + groupBy[i][x].augt + groupBy[i][x].sept  + groupBy[i][x].octt + groupBy[i][x].novt + groupBy[i][x].dect;
          var sumAccomp2 = sumAccomp + groupBy[i][x].jula + groupBy[i][x].auga + groupBy[i][x].sepa  + groupBy[i][x].octa + groupBy[i][x].nova + groupBy[i][x].deca;
        ...
        this.capped.push({"label": label,  "type": type, "pt_mid": total_mid_target, "pt": total_ye_target, "pa_mid": total_mid_capped, "pa": total_ye_capped})
      }
      ...
      console.log(this.capped);
        ...
        this.capped[9].pt_mid = this.capped[9].pt_mid + this.capped[5].pt_mid + this.capped[6].pt_mid + this.capped[7].pt_mid; // Research & Stations
        this.capped[9].pa_mid = this.capped[9].pa_mid + this.capped[5].pa_mid + this.capped[6].pa_mid + this.capped[7].pa_mid; // Research & Stations
        this.capped[9].pt = this.capped[9].pt + this.capped[5].pt + this.capped[6].pt + this.capped[7].pt; // Research & Stations
        this.capped[9].pa = this.capped[9].pa + this.capped[5].pa + this.capped[6].pa + this.capped[7].pa; // Research & Stations
  
        console.log(ous_physical);
      var ous_physical = this.capped.filter(obj => {
        return obj.type == 2
      }).sort((a, b) => (b.pa_mid / b.pt_mid) - (a.pa_mid / a.pt_mid));

      var banner_physical = this.capped.filter(obj => {
        return obj.type == 1
      }).sort((a, b) => (b.pa_mid / b.pt_mid) - (a.pa_mid / a.pt_mid));

      var special_physical = this.capped.filter(obj => {
        return obj.type == 3
      }).sort((a, b) => (b.pa_mid / b.pt_mid) - (a.pa_mid / a.pt_mid));
      ...
      for(var i = 0; i < ous_physical.length; i++) {
        this.dpsPhysicalMY2.push({y: (ous_physical[i].pa_mid / ous_physical[i].pt_mid ) *100 , label: ous_physical[i].label});
        this.dpsPhysicalYE2.push({y: (ous_physical[i].pa / ous_physical[i].pt ) *100 , label: ous_physical[i].label});
      }

      for(var i = 0; i < banner_physical.length; i++) {
        this.dpsPhysicalMY.push({y: (banner_physical[i].pa_mid / banner_physical[i].pt_mid ) *100 , label: banner_physical[i].label});
        this.dpsPhysicalYE.push({y: (banner_physical[i].pa / banner_physical[i].pt ) *100 , label: banner_physical[i].label});
      }

      for(var i = 0; i < special_physical.length; i++) {
        this.dpsPhysicalMY3.push({y: (special_physical[i].pa_mid / special_physical[i].pt_mid ) *100 , label: special_physical[i].label});
        this.dpsPhysicalYE3.push({y: (special_physical[i].pa / special_physical[i].pt ) *100 , label: special_physical[i].label});
      }
    ...
    this.pmisService.summary_all().subscribe(data => {
      ...
      this.summary[0].ft = this.summary[0].ft + this.summary[7].ft; // Rice & FPMA
      this.summary[1].ft = this.summary[1].ft + this.summary[11].ft; // HVCDP & FPMA
      this.summary[2].ft = this.summary[2].ft + this.summary[10].ft; // Corn & FPMA
      this.summary[4].ft = this.summary[4].ft + this.summary[15].ft; // Livestock & FPMA

      this.summary[0].fa = this.summary[0].fa + this.summary[7].fa; // Rice & FPMA
      this.summary[1].fa = this.summary[1].fa + this.summary[11].fa; // HVCDP & FPMA
      this.summary[2].fa = this.summary[2].fa + this.summary[10].fa; // Corn & FPMA
      this.summary[4].fa = this.summary[4].fa + this.summary[15].fa; // Livestock & FPMA

      this.summary[0].da = this.summary[0].da + this.summary[7].da; // Rice & FPMA
      this.summary[1].da = this.summary[1].da + this.summary[11].da; // HVCDP & FPMA
      this.summary[2].da = this.summary[2].da + this.summary[10].da; // Corn & FPMA
      this.summary[4].da = this.summary[4].da + this.summary[15].da; // Livestock & FPMA

      this.summary[22].ft = this.summary[22].ft + this.summary[23].ft + this.summary[24].ft + this.summary[25].ft; // Research & Stations
      this.summary[22].fa = this.summary[22].fa + this.summary[23].fa + this.summary[24].fa + this.summary[25].fa; // Research & Stations
      this.summary[22].da = this.summary[22].da + this.summary[23].da + this.summary[24].da + this.summary[25].da; // Research & Stations

      this.summary[14].ft = this.summary[14].ft + this.summary[16].ft; // RAED
      this.summary[14].fa = this.summary[14].fa + this.summary[16].fa; // RAED
      this.summary[14].da = this.summary[14].da + this.summary[16].da; // RAED

      this.summary[19].ft = this.summary[19].ft + this.summary[17].ft; // PMED & ICTU
      this.summary[19].fa = this.summary[19].fa + this.summary[17].fa; // PMED & ICTU
      this.summary[19].da = this.summary[19].da + this.summary[17].da; // PMED & ICTU

 
      this.resultProg = this.summary.filter(obj => {
        return obj.program_id < 6
      }).sort((a, b) => (b.fa / b.ft) - (a.fa / a.ft));

      this.resultOU = this.summary.filter(obj => {
        return obj.type == 2
      }).sort((a, b) => (b.fa / b.ft) - (a.fa / a.ft));

      this.resulSpecial = this.summary.filter(obj => {
        return obj.type == 3
      }).sort((a, b) => (b.fa / b.ft) - (a.fa / a.ft));
      ...
      for(var i = 0; i < 5; i++) {
        this.dpsAllocation.push({y: this.resultProg[i].ft, label: this.resultProg[i].name});
        this.dpsObligation.push({y: this.resultProg[i].fa, label: this.resultProg[i].name});
        this.dpsDisbursement.push({y: this.resultProg[i].da, label: this.resultProg[i].name});
        this.dpsObligationPercentage.push({y: (this.resultProg[i].fa / this.resultProg[i].ft ) *100 , label: this.resultProg[i].name});
        this.dpsDisbursementPercentage.push({y: (this.resultProg[i].da / this.resultProg[i].ft ) *100 , label: this.resultProg[i].name});
      ...
      for(var i = 0; i < this.resultOU.length; i++) {
        this.dpsAllocation2.push({y: this.resultOU[i].ft, label: this.resultOU[i].name});
        this.dpsObligation2.push({y: this.resultOU[i].fa, label: this.resultOU[i].name});
        this.dpsDisbursement2.push({y: this.resultOU[i].da, label: this.resultOU[i].name});
        this.dpsObligationPercentage2.push({y: (this.resultOU[i].fa / this.resultOU[i].ft ) *100 , label: this.resultOU[i].name});
        this.dpsDisbursementPercentage2.push({y: (this.resultOU[i].da / this.resultOU[i].ft ) *100 , label: this.resultOU[i].name});  
      ...
      for(var i = 0; i < this.resulSpecial.length; i++) {
        this.dpsAllocation3.push({y: this.resulSpecial[i].ft, label: this.resulSpecial[i].name});
        this.dpsObligation3.push({y: this.resulSpecial[i].fa, label: this.resulSpecial[i].name});
        this.dpsDisbursement3.push({y: this.resulSpecial[i].da, label: this.resulSpecial[i].name});
        this.dpsObligationPercentage3.push({y: (this.resulSpecial[i].fa / this.resulSpecial[i].ft ) *100 , label: this.resulSpecial[i].name});
        this.dpsDisbursementPercentage3.push({y: (this.resulSpecial[i].da / this.resulSpecial[i].ft ) *100 , label: this.resulSpecial[i].name});
  ...
  constructor(public pmisService: PmisService, private cd: ChangeDetectorRef) {
    ...
    this.columnDefs2 = [
      { headerName: 'Banner/Ous', field: 'username', pinned: 'left', width: 100 },
      { headerName: 'Obligation', width: 100, field: 'bed1.date_updated', cellRenderer: (data) => { return moment(data.data.bed1.date_updated).fromNow()}, 
      cellStyle: params => {
      ...
      { headerName: 'Physical', width: 100, field: 'bed2.date_updated', cellRenderer: (data) => { return moment(data.data.bed2.date_updated).fromNow()}, 
      cellStyle: params => {
        if (params.value === null) {
            //mark police cells as red
            return {color: 'red', backgroundColor: 'white'};
      ...
      { headerName: 'Disbursement', width: 100, field: 'bed3.date_updated',  cellRenderer: (data) => { return moment(data.data.bed3.date_updated).fromNow()},
      cellStyle: params => {
        if (params.value === null) {
            //mark police cells as red
            return {color: 'red', backgroundColor: 'white'};
 ...
 setTimeout(function(){  chart4.render(); }, 1000);
...
setTimeout(function(){  chart5.render(); }, 1000);
...
setTimeout(function(){  chart6.render(); }, 1000);
